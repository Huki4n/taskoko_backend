...
// The ProjectColumnFunc type is an adapter to allow the use of ordinary
// function as ProjectColumn mutator.
type ProjectColumnFunc func(context.Context, *ent.ProjectColumnMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ProjectColumnFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ProjectColumnMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ProjectColumnMutation", m)
}
