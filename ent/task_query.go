...
func (tq *TaskQuery) QueryColumn() *ProjectColumnQuery {
	query := (&ProjectColumnClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withColumn:      tq.withColumn.Clone(),
...
func (tq *TaskQuery) WithColumn(opts ...func(*ProjectColumnQuery)) *TaskQuery {
	query := (&ProjectColumnClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withColumn; query != nil {
		if err := tq.loadColumn(ctx, query, nodes, nil,
			func(n *Task, e *ProjectColumn) { n.Edges.Column = e }); err != nil {
			return nil, err
		}
	}
...
func (tq *TaskQuery) loadColumn(ctx context.Context, query *ProjectColumnQuery, nodes []*Task, init func(*Task), assign func(*Task, *ProjectColumn)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
		if nodes[i].project_column_tasks == nil {
			continue
		}
		fk := *nodes[i].project_column_tasks
	...
	query.Where(projectcolumn.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "project_column_tasks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
